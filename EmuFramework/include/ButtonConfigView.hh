...
{
	struct ResetMenuItem : public TextMenuItem//, public MultiPickView::Callback
	{
		constexpr ResetMenuItem(): v(0) { }
		ButtonConfigView *v;
		struct Confirm : public YesNoAlertView::Callback
		{
			constexpr Confirm(): v(0) { }
			ButtonConfigView *v;
		...
		void init(ButtonConfigView *v)
		{
			TextMenuItem::init("Unbind All");
			var_selfSet(v);
			confirm.v = v;
		}
		...
		void select(View *view, const InputEvent &e)
		{
			ynAlertView.init("Really unbind all keys in this category?", &confirm, !e.isPointer());
