...
void NavView::init(Target *target, ResourceFace *face)
{
	var_selfs(target);
	leftBtn.setZero();
	rightBtn.setZero();
...
void BasicNavView::init(Target *target, ResourceFace *face, ResourceImage *backRes, ResourceImage *closeRes,
		const GfxLGradientStopDesc *gradStop, uint gradStops)
{
	NavView::init(target, face);
...
/*void FSNavView::updateWDir()
{
	text.setString(FsSys::workDir());
}*/

/*void BasicNavView::drawTitle()
{
	using namespace Gfx;
	setColor(COLOR_WHITE);
	text.draw(gXPos(textRect, C2DO), gYPos(viewRect, C2DO), C2DO);
}*/

// FSNavView

void FSNavView::init(Target *target, ResourceFace *face, ResourceImage *backRes, ResourceImage *closeRes, bool singleDir)
{
	if(singleDir)
		backRes = 0;
	BasicNavView::init(target, face, backRes, closeRes, fsNavViewGrad, sizeofArray(fsNavViewGrad));
...
void FSPicker::init(const char *path, ResourceImage *backRes, ResourceImage *closeRes, FsDirFilterFunc filter,  bool singleDir, ResourceFace *face)
{
	...
	navV.init(this, face, backRes, closeRes, singleDir);
	loadDir(path);
}
...
void FSPicker::loadDir(const char *path)
{
	...
	if(dir.numEntries())
	{
		text = realloc(text, dir.numEntries());//TextMenuItem::realloc(text, dir.numEntries());
		if(text == 0)
		{
