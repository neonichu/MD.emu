...
public:
	...
	bindTextureState(0), bindTextureTargetState(GL_TEXTURE_2D),
...
#ifdef CONFIG_CXX11
	, GL_TEXTURE_ENV_GL_TEXTURE_ENV_COLOR_state({ 0, 0, 0, 0 })
	, colorState({ 1, 1, 1, 1 })
#endif
	{
#ifndef CONFIG_CXX11
	...
	GLuint bindTextureState;
	// TODO: support targets other than GL_TEXTURE_2D
	GLenum bindTextureTargetState;

	void bindTexture(GLenum target, GLuint texture)
	{
		if(texture != bindTextureState)
		{
			//logMsg("binding texture %d", texture);
			glBindTexture(target, texture);
			bindTextureState = texture;
		}
	}
	...
	void deleteTextures(GLsizei n, const GLuint *textures)
	{
		...
		iterateTimes(n, i)
		{
			if(textures[i] == bindTextureState)
				bindTextureState = 0;
		}
	...
	struct GLStateCaps
	{
		...
		bool GL_ALPHA_TEST_state;
		bool GL_DEPTH_TEST_state;
		bool GL_FOG_state;
		bool GL_BLEND_state;
		bool GL_SCISSOR_TEST_state;
		bool GL_CULL_FACE_state;
		bool GL_TEXTURE_2D_state;
		#ifndef CONFIG_GFX_OPENGL_ES
		// extensions
		bool GL_MULTISAMPLE_ARB_state;
		#endif
	} stateCap;
	...
	struct GLClientStateCaps
	{
		...
		bool GL_TEXTURE_COORD_ARRAY_state;
		bool GL_COLOR_ARRAY_state;
	} clientStateCap;
