...
static void handleKeyEvent(int key, int down)
{
	assert((uint)key < Key::COUNT);
	if(likely(onInputEventHandler != 0))
		onInputEventHandler(onInputEventHandlerCtx, InputEvent(0, InputEvent::DEV_KEYBOARD, key & 0xff, down ? INPUT_PUSHED : INPUT_RELEASED));
}
