...
void md_cart_init(void)
{
  ...
  ***********************************************/
  ...
  {
    /* cartridge ROM */
    mm68k.memory_map[i].base     = cart.rom + ((i<<16) & cart.mask);
  ...
  for (i=0x40; i<0x80; i++)
  {
    ...
    mm68k.memory_map[i].read8    = m68k_read_bus_8;
    mm68k.memory_map[i].read16   = m68k_read_bus_16;
    mm68k.memory_map[i].write8   = m68k_unused_8_w;
    mm68k.memory_map[i].write16  = m68k_unused_16_w;
  ...
  ***********************************************/
  ...
  if (sram.on)
  {
    ...
    {
      /* Serial EEPROM */
      mm68k.memory_map[eeprom.type.sda_out_adr >> 16].read8  = eeprom_read_byte;
      mm68k.memory_map[eeprom.type.sda_out_adr >> 16].read16 = eeprom_read_word;
      mm68k.memory_map[eeprom.type.sda_in_adr >> 16].read8   = eeprom_read_byte;
      mm68k.memory_map[eeprom.type.sda_in_adr >> 16].read16  = eeprom_read_word;
      mm68k.memory_map[eeprom.type.scl_adr >> 16].write8     = eeprom_write_byte;
      mm68k.memory_map[eeprom.type.scl_adr >> 16].write16    = eeprom_write_word;
    ...
    else
    {
      ...
      {
        mm68k.memory_map[sram.start >> 16].base    = sram.sram;
        mm68k.memory_map[sram.start >> 16].read8   = NULL;
        mm68k.memory_map[sram.start >> 16].read16  = NULL;
        mm68k.memory_map[sram.start >> 16].write8  = NULL;
        mm68k.memory_map[sram.start >> 16].write16 = NULL;
  ...
  ***********************************************/
  ...
  if (strstr(rominfo.international,"Virtua Racing") != NULL)
  {
    ...
    mm68k.memory_map[0x30].base    = svp->dram;
    mm68k.memory_map[0x30].read16  = NULL;
    mm68k.memory_map[0x30].write16 = svp_write_dram;

    mm68k.memory_map[0x31].base    = svp->dram + 0x10000;
    mm68k.memory_map[0x31].read16  = NULL;
    mm68k.memory_map[0x31].write16 = svp_write_dram;

    mm68k.memory_map[0x39].read16  = svp_read_cell_1;
    mm68k.memory_map[0x3a].read16  = svp_read_cell_2;
  }
  #endif
  ...
  ***********************************************/
      ...
      (strstr(rominfo.product,"T-120096")    != NULL))                                    /* Micro Machines 2 */
  {
    if (cart.romsize <= 0x380000)  /* just to be sure (checksum might not be enough) */
    {
      ...
      mm68k.memory_map[0x38].read16  = jcart_read;
      mm68k.memory_map[0x38].write16 = jcart_write;
      mm68k.memory_map[0x3f].read16  = jcart_read;
      mm68k.memory_map[0x3f].write16 = jcart_write;
    }
  }
  ...
  ***********************************************/
  ...
  switch (config.lock_on)
  {
    ...
    case TYPE_SK:
    {
      ...
      for (i=0x00; i<0x20; i++)
      {
        mm68k.memory_map[i].base = (cart.rom + 0x600000) + (i<<16);
      }
  ...
  ***********************************************/
  ...
  for (i=0; i < CART_CNT + 1; i++)
  {
        ...
        (rominfo.realchecksum == rom_database[i].chk_2))
    {
      ...
      while (j <= rom_database[i].bank_end)
      {
        if (cart.hw.regs_r)
        {
          mm68k.memory_map[j].read8    = cart.hw.regs_r;
          mm68k.memory_map[j].read16   = cart.hw.regs_r;
          zbank_memory_map[j].read    = cart.hw.regs_r;
        }
        if (cart.hw.regs_w)
        {
          mm68k.memory_map[j].write8   = cart.hw.regs_w;
          mm68k.memory_map[j].write16  = cart.hw.regs_w;
  ...
  if (cart.hw.realtec)
  {
    ...
    for (i=0x00; i<0x40; i++)
    {
      mm68k.memory_map[i].base = cart.rom + 0x900000;
    }
  }

#if M68K_EMULATE_ADDRESS_ERROR == OPT_ON
  /* default behavior */
  emulate_address_error = config_addr_error;
#endif
  ...
  {
    /* 128K ROM Bankswitch */
	  mm68k.memory_map[0].write8   = mapper_wukong_w;
    zbank_memory_map[0].write   = mapper_wukong_w;

    /* 8K SRAM */
    mm68k.memory_map[0x3C].base    = sram.sram;
    mm68k.memory_map[0x3C].read8   = NULL;
    mm68k.memory_map[0x3C].read16  = NULL;
    mm68k.memory_map[0x3C].write8  = NULL;
    mm68k.memory_map[0x3C].write16 = NULL;
...
void md_cart_reset(int hard_reset)
{
  ...
  cart.base = mm68k.memory_map[0].base;
}
...
int md_cart_context_load(uint8 *state)
{
  ...
  for (i=0; i<0x40; i++)
  {
    ...
    {
      /* SRAM */
      mm68k.memory_map[i].base = sram.sram;
    }
    else
    {
      /* ROM */
      mm68k.memory_map[i].base = cart.rom + (offset << 16);
    }
  }
...
static void mapper_sega_w(uint32 data)
{
  ...
  if (data & 1)
  {
    if (sram.on)
    {
      ...
      mm68k.memory_map[0x20].base = sram.sram;
      mm68k.memory_map[0x20].write8  = NULL;
      mm68k.memory_map[0x20].write16 = NULL;
      zbank_memory_map[0x20].write  = NULL;

      /* Backup RAM write protection */
      if (data & 2)
      {
        mm68k.memory_map[0x20].write8  = m68k_unused_8_w;
        mm68k.memory_map[0x20].write16 = m68k_unused_16_w;
    ...
    if ((cart.special & HW_LOCK_ON) && (config.lock_on == TYPE_SK))
    {
      ...
      for (i=0x30; i<0x40; i++)
      {
        mm68k.memory_map[i].base = (cart.rom + 0x800000) + ((i & 3) << 16);
      }
    }
  }
  else
...
static void mapper_ssf2_w(uint32 address, uint32 data)
{
  ...
  if (dst)
  {
    ...
    for (i=0; i<8; i++)
    {
      mm68k.memory_map[dst++].base = src + (i<<16);
    }
  }
}
...
static void mapper_wukong_w(uint32 address, uint32 data)
{
  ...
  if (data & 0x80)
  {
    ...
    for (i=0x20; i<0x3C; i++)
    {
    	mm68k.memory_map[i].base = cart.rom + (((i&0x1F)<<16) & cart.mask);
    }
  }
  else
  {
  ...
  switch (address)
  {
    ...
    case 0x400000:  
    {
      ...
      if (cart.hw.regs[2])
      {
        ...
        int i;
        for (i=0x00; i<0x40; i++)
          mm68k.memory_map[i].base = &cart.rom[(base + (i % cart.hw.regs[2])) << 16];
      }
      return;
    }
  }
  ...
  if (data)
  {
    for (i=0; i<0x10; i++)
    {
      /* Remap to unused ROM area  */
    	mm68k.memory_map[i].base = &cart.rom[0x400000 + (i << 16)];

      /* address = address OR (value << 15) */
      memcpy(mm68k.memory_map[i].base, cart.rom + ((i << 16) | (data & 0x3f) << 15), 0x8000);
      memcpy(mm68k.memory_map[i].base + 0x8000, cart.rom + ((i << 16) | ((data | 1) & 0x3f) << 15), 0x8000);
    }
  }
  else
  {
...
static void mapper_64k_w(uint32 data)
{
  ...
  if (data)
  {
    ...
    for (i=0; i<16; i++)
    {
      mm68k.memory_map[i].base = &cart.rom[(data & 0xf) << 16];
    }
  }
  else
  {
    ...
    for (i=0; i<16; i++)
    {
      mm68k.memory_map[i].base = &cart.rom[(i & 0xf) << 16];
    }
  }
}
...
static void mapper_64k_multi_w(uint32 address)
{
  ...
  for (i=0; i<64; i++)
  {
    mm68k.memory_map[i].base = &cart.rom[((address++) & 0x3f) << 16];
  }
}
