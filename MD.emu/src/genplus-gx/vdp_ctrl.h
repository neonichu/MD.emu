...
union RamU
{
	...
	// TODO: can use GCC 4.7's __builtin_assume_aligned built-in here
	uint32 &getL(uint byteAddr) __attribute__ ((always_inline))
	{
		//assert((byteAddr & 0x3) == 0);
		return *((uint32*)&b[byteAddr]);
		//return l[byteAddr >> 2];
	}

	uint16 &getS(uint byteAddr) __attribute__ ((always_inline))
	{
		//assert((byteAddr & 0x1) == 0);
		return *((uint16*)&b[byteAddr]);
		//return s[byteAddr >> 1];
	}
};
