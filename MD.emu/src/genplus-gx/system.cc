...
int audio_update (void)
{
  ...
  size &= ~7;
	...
	bool doPCM = (scdPCM.control & 0x80) && scdPCM.enabled;
	if(usingSCD && doPCM)
	{
...
{
	#ifndef NO_SCD
		if(usingSCD) sm68k.cycleCount = mm68k.cycleCount;
	#endif
	m68k_run(mm68k, cycles);
	#ifndef NO_SCD
		if(usingSCD && (scdBusreq&3) == 1)
		{
			m68k_run(sm68k, cycles + MCYCLES_PER_LINE);
		}
	#endif
}
...
static void system_frame_md(int do_skip, uint renderGfx)
{
  ...
  do
  {
    ...
    runM68k(mcycles_vdp + MCYCLES_PER_LINE);
    if (zstate == 1)
    {
